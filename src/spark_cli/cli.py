...
def resolve_llm_doctor_target(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for role in role_order:
        ...
        if provider in {"openai", "zai", "kimi", "minimax", "openrouter", "huggingface"}:
            ...
            # Fallback: the key may be stored as a conventional env var
            # (e.g. OPENAI_API_KEY) rather than in Spark's secret store.
            if not api_key:
...
def provider_test_payload(*, role: str = "chat", provider: str | None = None) -> dict[str, Any]:
    try:
        ...
        # from this process" so users see an accurate repair message.
        ...
        if key_configured_in_setup:
            return {
                ...
                "model": role_state.get("model"),
                "detail": (
                    f"Provider {configured_provider} is configured in Spark setup, but the API key "
                    "is not reachable from the test probe in this environment "
                    "(not in Spark secret store or OPENAI_API_KEY env var). "
                    "Run `spark providers status` to confirm the provider is ready."
                ),
                "repair": "spark providers status",
            }
        return {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    os_compile_parser.add_argument("--desktop", default=str(Path.home() / "Desktop"), help="Developer Desktop root for local Spark repo checkouts. On server installs where ~/Desktop is absent, authority sources are automatically resolved from SPARK_HOME.")
    ...
    os_capabilities_parser.add_argument("--desktop", default=str(Path.home() / "Desktop"), help="Developer Desktop root for local Spark repo checkouts. On server installs where ~/Desktop is absent, authority sources are automatically resolved from SPARK_HOME.")
    ...
    os_authority_parser.add_argument("--desktop", default=str(Path.home() / "Desktop"), help="Developer Desktop root for local Spark repo checkouts. On server installs where ~/Desktop is absent, authority sources are automatically resolved from SPARK_HOME.")
    ...
    os_trace_parser.add_argument("--desktop", default=str(Path.home() / "Desktop"), help="Developer Desktop root for local Spark repo checkouts. On server installs where ~/Desktop is absent, authority sources are automatically resolved from SPARK_HOME.")
    ...
    os_memory_parser.add_argument("--desktop", default=str(Path.home() / "Desktop"), help="Developer Desktop root for local Spark repo checkouts. On server installs where ~/Desktop is absent, authority sources are automatically resolved from SPARK_HOME.")
    ...
    providers_parser.set_defaults(func=cmd_providers, providers_command=None, json=False, role="chat", provider=None)
    ...
    recommend_sub = recommend_parser.add_subparsers(dest="recommend_command", required=False)
    recommend_parser.set_defaults(func=cmd_recommend, recommend_command=None, json=False)
